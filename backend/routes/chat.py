...
_ON_TOPIC_RE = re.compile(
    ...
    r"outage|issue|problem|cause|impact|affect|occur|happen|"
    r"replac|replicat|corrupt|disconnect|packet|datanode|namenode|"
    r"where\s+(does|did|is|are|do|was|were)|"
    r"what\s+(caused|happened|went\s+wrong|is|are|does)|"
    r"why\s+(did|is|are|was|were)|"
    r"how\s+(many|often|severe|did|does)|"
    r"which\s+(session|node|block|error)|"
...
def _build_chat_context(session_id: Optional[str]) -> str:
    ...
    lines = [
        ...
        "- Use **bold** for key terms, block IDs, node addresses, and important values",
        "- Use - bullet points for lists of findings or items",
        "- Use 1. 2. 3. for sequential steps or ordered observations",
        "- Use --- as a divider between major sections",
        "- End with a brief **Severity:** line when relevant",
