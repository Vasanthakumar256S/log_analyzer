...
async def tryout(
    ...
    """Quick demo: run the full demo_hdfs.log sample file."""
    if not _DEMO_FILE.exists():
        raise HTTPException(
            status_code=404,
            detail=f"Demo file not found: {_DEMO_FILE}. "
                   "Place demo_hdfs.log in data/raw/ to enable the tryout mode."
        )
    if app_state.job["step"] not in ("idle", "ready", "error"):
        raise HTTPException(status_code=409, detail="A pipeline is already running.")

    session_id = str(uuid4())
    app_state.set_step("parsing", "Try-out mode: running demo_hdfs.log…", 5)
    background_tasks.add_task(
        _run_pipeline, _DEMO_FILE, _DEMO_DATASET, None, session_id, "demo_hdfs.log", current_user
    )
    return {"status": "accepted", "mode": "tryout", "filename": "demo_hdfs.log",
            "session_id": session_id}
